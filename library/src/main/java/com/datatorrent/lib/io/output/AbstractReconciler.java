...
/**
 * This base operator queues input tuples for each window and asynchronously
 * processes them after the window is committed.
 *
 * The operator holds all the tuple info in memory until the committed window
 * and then calls the processCommittedData method to give an opportunity to
 * process tuple info from each committed window.
 *
 * This operator can be implemented to asynchronously read and process data that
 * is being written by current application.
 *
 * Use case examples: write to relational database, write to an external queue
 * etc without blocking the dag i/o.
 *
 * @param <INPUT>
 *          input type
 * @param <QUEUETUPLE>
    ...
    implements CheckpointListener, IdleTimeHandler
{
  ...
  /**
   * The implementation class should call this method to enqueue output once
   * input is converted to queue input.
   *
   * The queueTuple is processed once the window in which queueTuple is enqueued
   * is committed.
   *
   * @param queueTuple
   */
  ...
  /**
   * This method is called once the window in which queueTuple was created is
   * committed. Implement this method to define the functionality to synchronize
   * data.
   *
   * @param queueInput
   */
